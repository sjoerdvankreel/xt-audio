...
#define XT_IMPLEMENT_DEVICE_LIST(s)                                            \
  ...
  XtFault GetDefaultId(XtBool output, XtBool* valid, char* buffer, int32_t* size) const override;
...
struct XtDeviceList
{
  ...
  virtual XtFault GetDefaultId(XtBool output, XtBool* valid, char* buffer, int32_t* size) const = 0;
};
