...
#define XT_IMPLEMENT_DEVICE_LIST(s)                                                  \
  ...
  XtFault GetCapabilities(char const* id, XtDeviceCaps* capabilities) const override final
...
struct XtDeviceList
{
  ...
  virtual XtFault GetCapabilities(char const* id, XtDeviceCaps* capabilities) const = 0;
};
