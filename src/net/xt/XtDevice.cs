...
namespace Xt
{
    ...
    {
        [DllImport("xt-audio")]
        static extern void XtDeviceDestroy(IntPtr d);
        [DllImport("xt-audio")]
        static extern IntPtr XtDeviceGetHandle(IntPtr d);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceShowControlPanel(IntPtr d);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceGetMix(IntPtr d, out bool valid, out XtMix mix);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceGetChannelCount(IntPtr d, bool output, out int count);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceSupportsAccess(IntPtr d, bool interleaved, out bool supports);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceSupportsFormat(IntPtr d, in XtFormat format, out bool supports);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceGetBufferSize(IntPtr d, in XtFormat format, out XtBufferSize size);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceGetChannelName(IntPtr d, bool output, int index, [Out] byte[] buffer, ref int size);
        [DllImport("xt-audio")]
        static extern ulong XtDeviceOpenStream(IntPtr d, in DeviceStreamParams @params, IntPtr user, out IntPtr stream);
        ...
        => HandleAssert(XtDeviceGetHandle(_d));
        public void ShowControlPanel()
        => HandleError(XtDeviceShowControlPanel(_d));
        public XtBufferSize GetBufferSize(in XtFormat format)
        => HandleError(XtDeviceGetBufferSize(_d, in format, out var r), r);
        public int GetChannelCount(bool output)
        => HandleError(XtDeviceGetChannelCount(_d, output, out var r), r);
        public bool SupportsFormat(in XtFormat format)
        => HandleError(XtDeviceSupportsFormat(_d, in format, out var r), r);
        public bool SupportsAccess(bool interleaved)
        => HandleError(XtDeviceSupportsAccess(_d, interleaved, out var r), r);
        public XtMix? GetMix()
        => HandleError(XtDeviceGetMix(_d, out var v, out var r)) && v ? r : (XtMix?)null;

        public void Dispose()
        {
            HandleAssert(() => XtDeviceDestroy(_d));
            _d = IntPtr.Zero;
        }
